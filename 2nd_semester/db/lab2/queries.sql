...
SELECT НАИМЕНОВАНИЕ
FROM
(SELECT НАИМЕНОВАНИЕ, count(*)
FROM Н_ДИСЦИПЛИНЫ
GROUP BY НАИМЕНОВАНИЕ) cte;
...
SELECT ФИО
FROM (SELECT p.ФАМИЛИЯ || ' ' || p.ИМЯ || ' ' || p.ОТЧЕСТВО AS ФИО, count(p.ИД)
    FROM Н_ЛЮДИ p
    LEFT JOIN Н_УЧЕНИКИ s ON s.ЧЛВК_ИД = p.ИД
    WHERE s.СОСТОЯНИЕ IS NULL
    GROUP BY 1) cte;
